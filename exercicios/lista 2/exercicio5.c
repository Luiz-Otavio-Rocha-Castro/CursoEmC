...
int main(){
    ...
    printf("Digite sua altura(metros) e seu peso(kg): ");
    scanf("%f%f", &altura, &peso );
    imc = peso / (altura * altura);
    if(imc < 18.5){
        printf("Seu IMC: %.2f e voce se encontra abaixo do peso\n", imc);
    }
    else if (imc >= 18.5 && imc < 25){
        printf("Seu IMC: %.2f e voce se encontra no peso normal\n", imc);
    }
    else if(imc >= 25 && imc < 30){
        printf("Seu IMC: %.2f e voce se encontra sobrepeso\n", imc);
    }
    else if (imc >=30 && imc < 35){
        printf("Seu IMC: %.2f e voce se encontra na obesidade de grau 1\n", imc);
    }
    else if(imc >= 35 && imc < 40){
        printf("Seu IMC: %.2f e voce se encontra na obesidade de grau 2\n", imc);
    }
    else{
        printf("Seu IMC: %.2f e voce se encontra na obesidade de grau 3\n", imc);
    }
